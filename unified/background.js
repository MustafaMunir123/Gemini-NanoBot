                    ...
                    if (result.voiceControlEnabled !== false) {
                        ...
                        chrome.tabs.sendMessage(tab.id, {
                            ...
                            if (chrome.runtime.lastError) {
                                console.log("Keyboard shortcut message failed:", chrome.runtime.lastError);
                                // Fallback: inject content script if needed
                                chrome.scripting.executeScript({
                                    target: { tabId: tab.id },
                                    files: ["content.js"]
                                }).catch((err) => {
                                    console.error("Failed to inject script for keyboard shortcut:", err);
                                });
                            } else {
                                console.log("Keyboard shortcut message sent successfully");
                            }
                        });
                    } else {
                        console.log("Voice control is disabled, ignoring keyboard shortcut");
                    }
                });
            }
        });
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (changeInfo.status === 'complete' && tab.url && !tab.url.startsWith('chrome://')) {
        console.log("Tab updated, checking if content script needs injection:", tab.url);
        ...
        chrome.storage.local.get(['voiceControlEnabled', 'floatingIndicatorEnabled'], (result) => {
            const needsInjection = result.voiceControlEnabled !== false || result.floatingIndicatorEnabled === true;
            ...
            if (needsInjection) {
                chrome.scripting.executeScript({
                    ...
                    console.log("Content script injection failed (likely already injected):", err);
                });
            }
        });
    }
                ...
                tabs.forEach(tab => {
                    if (tab.url && !tab.url.startsWith('chrome://')) {
                        chrome.tabs.sendMessage(tab.id, {
                            ...
                            floatingIndicatorEnabled: changes.floatingIndicatorEnabled?.newValue
                        }, (response) => {
                            if (chrome.runtime.lastError) {
                                console.log("State change message failed for tab:", tab.id, chrome.runtime.lastError);
                            }
                        });
                    }
                });
